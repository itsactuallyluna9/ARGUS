...
articles <- read.csv("data/article_data.csv")
fact_checks <- read.csv("data/fact_check_data.csv")
colnames(fact_checks)
```
...
bias_scores_by_source()
accuracy_completeness_scores_by_source()
```

```{r}
#scores by timeframe (density)
